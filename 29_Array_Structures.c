...
{	//main function body starts
     ...
     strcpy(record[0].name, "Pranoy");	//using strcpy to store value Pranoy
     record[0].percentage = 99.0;	//assigning value
 
     // 2nd student's record         
     record[1].id=2;	//assigning value using dot operator
     strcpy(record[1].name, "Rohan");	//using strcpy to store value Rohan
     record[1].percentage = 99.1;	//assigning value
 
     // 3rd student's record
     record[2].id=3;	//assigning value using dot operator
     strcpy(record[2].name, "Shubam");	//using strcpy to store value shubam
